	...
	void FOculusXRAnchorManager::OnPollEvent(ovrpEventDataBuffer* EventDataBuffer, bool& EventPollResult)
	{
		...
		switch (buf.EventType) {
			...
			case ovrpEventType_SpaceEraseComplete:
			{
				...
				UE_LOG(LogOculusXRAnchors, Log, TEXT("ovrpEventType_SpaceEraseComplete  Request ID: %llu  --  Result: %d  -- UUID: %s"), SpaceEraseEvent.requestId, SpaceEraseEvent.result, *UOculusXRAnchorBPFuctionLibrary::AnchorUUIDToString(SpaceEraseEvent.uuid.data));
